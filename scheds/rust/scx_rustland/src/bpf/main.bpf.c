...
/*
 * Return the DSQ ID associated to a CPU.
 */
...
static void dispatch_task(struct task_struct *p, u64 dsq_id, u64 enq_flags)
{
	...
	default:
		 ...
		 *
		 * In the future we may want to check the cpumask from
		 * user-space, in order to assign a valid CPU (even if without
		 * some kind of synchronization this process is going to be
		 * racy).
		 */
 ...
 * dispatched to a custom DSQ (either the shared one or the one associated to
 * the CPU selected by the user-space scheduler).
 *
